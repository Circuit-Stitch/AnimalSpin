...
"""
import csv, os, sys, hashlib, shutil, datetime
...
# 1. blocklist targets: archive the audio into the known-bads corpus + log provenance.
#    Dedupe by sha256 so a reused slot gets a fresh row per distinct bad clip.
...
for b in targets:
    ...
    h = F.archive_reject(f, r[1]) if os.path.exists(f) else "MISSING"
    if h not in have:
        rbody.append([path(b), r[1], r[2], h, TODAY])
        have.add(h)
...
for animal, slots in sorted(by_animal.items()):
    ...
    picks = F.fetch_animal(animal, len(slots), os.path.join(out, animal), seen, rej_h, known)

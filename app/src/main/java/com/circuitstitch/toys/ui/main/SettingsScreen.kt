...
fun SettingsScreen(onDone: () -> Unit, vm: SettingsViewModel = viewModel()) {
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            val creditsSize =
                with(LocalDensity.current) { dimensionResource(R.dimen.credits_text_size).toSp() }
            val creditsLineHeight =
                with(LocalDensity.current) { dimensionResource(R.dimen.credits_line_height).toSp() }
